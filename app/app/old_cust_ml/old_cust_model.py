...
class OldCustModel:
    ...
    def __init__(self):
        # 加载模型相关文件
        self.lgb_model = joblib.load('app/app/old_cust_ml/static/oldModel.pkl')
        self.lr_model = joblib.load('app/app/old_cust_ml/static/oldModel_lr.pkl')
        self.hxy_feat = load_txt_feature('app/app/old_cust_ml/static/oldFeat.txt')
        self.cl_feat = load_txt_feature('app/app/old_cust_ml/static/oldFeat2.txt')
        self.comp = pd.read_excel('app/app/old_cust_ml/static/comp.xlsx', names=['package', 'name'])
        self.all_feat = load_txt_feature('app/app/old_cust_ml/static/old_cust_20220104.txt')
    ...
    def __get_feat(self,data:dict):
        ...
        if len(data['app_list']) == 0 or len(data['add_list']) == 0:  # 为空
            log.logger.warning('app_list or add_list is empty, fill cl feat 0')
            all_tag_dict = {k: 0 for k in self.cl_feat}
